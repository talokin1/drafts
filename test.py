...
# 1. Створюємо синтетичні фічі ПЕРЕД агрегацією
df = train_dataset.copy()
df['leasing_to_price_ratio'] = df['min_month_leasing_pay'] / df['price_usd']
df['is_luxury_car'] = (df['mark_group'] == 'Luxury').astype(int)

# 2. Правила агрегації
agg_funcs = {
    'is_hnwi': 'max',  # Якщо хоча б раз помічений як HNWI, клієнт є HNWI
    'price_usd': ['count', 'mean', 'max', 'sum'], # count - це кількість авто
    ...
    'has_report': 'max',
    'is_abroad': 'max',
    # Для категоріальних беремо найчастіше значення (моду)
...
# 4. Вирівнюємо багаторівневі назви колонок (напр. 'price_usd_mean')
df_client.columns = ['_'.join(col).strip() if col[1] else col[0] for col in df_client.columns.values]

# Перейменовуємо таргет та фічу кількості машин
