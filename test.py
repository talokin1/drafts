...
TARGET_NAME = "CURR_ACC"              # зміни, якщо таргет має іншу назву
ID_COL = "IDENTIFYCODE"
SEGMENT_COL = "FIRM_TYPE"

# meaningful active threshold
# для такого розподілу я б не ставив 0, бо 25% = 0.011, median = 66.79
# тобто багато технічного/майже нульового income
ACTIVE_THRESHOLD = 100

# probability threshold НЕ використовується для фінального prediction
# тільки для business flag / diagnostics
...
plt.title("Final Expected Potential: True vs Predicted")
plt.show()
...
plot_df = decile_report.copy()
plot_df["pred_decile"] = plot_df["pred_decile"].astype(str)

plt.plot(plot_df["pred_decile"], plot_df["true_sum"], marker="o", label="True sum")
plt.plot(plot_df["pred_decile"], plot_df["pred_sum"], marker="o", label="Predicted sum")
...
def assign_potential_bucket(pred):
    pred = np.array(pred)

    conditions = [
        pred <= 0,
        (pred > 0) & (pred <= 1_000),
        (pred > 1_000) & (pred <= 5_000),
        (pred > 5_000) & (pred <= 20_000),
        (pred > 20_000) & (pred <= 100_000),
        pred > 100_000
    ]
...
validation_results["LIABILITIES_POTENTIAL_BUCKET"] = assign_potential_bucket(
    validation_results["Predicted"].values
)

bucket_report = group_diagnostics(validation_results, ["LIABILITIES_POTENTIAL_BUCKET"])
